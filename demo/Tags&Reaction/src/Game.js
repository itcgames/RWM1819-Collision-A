class Game
{
    ...
    init() {
        ...
        gameNs.game.collisionManager.addPolygonCollider(new PolygonCollider(new Polygon([new Vector2(400,50), new Vector2(420, 50), new Vector2(420, 150), new Vector2(400, 150)]), ['door'], ['player']));
        ...
        document.addEventListener('keydown', function(event) {
            if(event.keyCode == 37) {
                ...
                gameNs.game.player.shape.getVertices().forEach(vertex => {
                    var collisionTags = CollisionManager.GetTagsOfCollided(gameNs.game.collisionResults[0]);
